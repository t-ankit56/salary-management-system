...
- **Editing salary or employment period dates** — corrections amend amounts only; dates are immutable once a period exists. Altering one boundary alone produces an overlap or a gap, so a date correction is inherently a multi-period operation, needing its own validation, logging and an interface showing which adjacent periods move and what range is permitted. Disproportionate for a rare data-entry error. A period entered with wrong dates cannot be corrected through the application in this version.
